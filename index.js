...
bot.on("new_chat_members", async(itsuki) => {
    var message = itsuki.message
    ...
    for (x of message.new_chat_members) {
        ...
        await itsuki.replyWithPhoto({url: `https://hardianto-chan.herokuapp.com/api/tools/welcomer?nama=${full_name}&namaGb=${groupname}&pepeGb=${pp_group}&totalMem=200&pepeUser=${pp_user}&bege=https://telegra.ph/file/f69e975d76d0366820742.jpg&apikey=hardianto`})
        await itsuki.reply('Hello There!')
    }
})

bot.on("left_chat_member", async(itsuki) => {
    var message = itsuki.message
    var pp_group = await tele.getPhotoProfile(message.chat.id)
    ...
    await itsuki.replyWithPhoto({url: `https://hardianto-chan.herokuapp.com/api/goodbye3?profile=${pp_user}&name=${full_name}&bg=https://telegra.ph/file/f69e975d76d0366820742.jpg&namegb=${groupname}&member=200`})
    await itsuki.reply('Goodbye, undefined')
})

bot.command('start', async(itsuki) => {
    user = tele.getUser(itsuki.message.from)
    await help.start(itsuki, user.full_name)
    await itsuki.deleteMessage()
})

bot.command('help', async(itsuki) => {
    user = tele.getUser(itsuki.message.from)
    await help.help(itsuki, user.full_name, itsuki.message.from.id.toString())
})

bot.on("callback_query", async(itsuki) => {
    cb_data = itsuki.callbackQuery.data.split("-")
    user_id = Number(cb_data[1])
    if (itsuki.callbackQuery.from.id != user_id) return itsuki.answerCbQuery("Sorry, You do not have the right to access this button.", { show_alert: true })
    callback_data = cb_data[0]
    user = tele.getUser(itsuki.callbackQuery.from)
    const isGroup = itsuki.chat.type.includes("group")
    const groupName = isGroup ? itsuki.chat.title : ""
    if (!isGroup) console.log(chalk.whiteBright("- "), chalk.cyanBright("[ USER ]"), chalk.whiteBright(callback_data), chalk.greenBright("from"), chalk.whiteBright(user.full_name))
    if (isGroup) console.log(chalk.whiteBright("- "), chalk.cyanBright("[ GROUP ]"), chalk.whiteBright(callback_data), chalk.greenBright("from"), chalk.whiteBright(user.full_name), chalk.greenBright("in"), chalk.whiteBright(groupName))
    if (callback_data == "help") return await help[callback_data](itsuki, user.full_name, user_id)
    await help[callback_data](itsuki, user_id.toString())
})

bot.on("message", async(itsuki) => {
    try {
        const body = itsuki.message.text || itsuki.message.caption || ""
        ...
        const command = comm
        const args = await tele.getArgs(itsuki)
        const user = tele.getUser(itsuki.message.from)

        const reply = async(text) => {
            for (var x of range(0, text.length, 4096)) {
                return await itsuki.replyWithMarkdown(text.substr(x, 4096), { disable_web_page_preview: false })
            }
        }
        ...
        const isImage = itsuki.message.hasOwnProperty("photo")
        const isVideo = itsuki.message.hasOwnProperty("video")
        const isAudio = itsuki.message.hasOwnProperty("audio")
        const isSticker = itsuki.message.hasOwnProperty("sticker")
        const isContact = itsuki.message.hasOwnProperty("contact")
        const isLocation = itsuki.message.hasOwnProperty("location")
        const isDocument = itsuki.message.hasOwnProperty("document")
        const isAnimation = itsuki.message.hasOwnProperty("animation")
        const isMedia = isImage || isVideo || isAudio || isSticker || isContact || isLocation || isDocument || isAnimation

        const quotedMessage = itsuki.message.reply_to_message || {}
        ...
        const isQuoted = itsuki.message.hasOwnProperty("reply_to_message")
        ...
        var file_id = ""
        if (isQuoted) {
            file_id = isQuotedImage ? itsuki.message.reply_to_message.photo[itsuki.message.reply_to_message.photo.length - 1].file_id :
                isQuotedVideo ? itsuki.message.reply_to_message.video.file_id :
                isQuotedAudio ? itsuki.message.reply_to_message.audio.file_id :
                isQuotedDocument ? itsuki.message.reply_to_message.document.file_id :
                isQuotedAnimation ? itsuki.message.reply_to_message.animation.file_id : ""
        }
        var mediaLink = file_id != "" ? await tele.getLink(file_id) : ""

        switch (command) {
            case 'help':
                await itsuki.replyWithChatAction("typing")
                await reply('P L E A S E  W A I T !')
                await help.help(itsuki, user.full_name, itsuki.message.from.id.toString())
                break
            ...
            case 'alquranaudio':
                ...
                surah = args[0]
                await itsuki.replyWithAudio({ url: `https://api.lolhuman.xyz/api/quran/audio/${surah}?apikey=${apikey}` })
                break
            case 'asmaulhusna':
                ...
                await fetchJson(`https://api.lolhuman.xyz/api/ytsearch?apikey=${apikey}&query=${args.join(" ")}`)
                    .then(async(result) => {
                        await fetchJson(`https://api.lolhuman.xyz/api/ytaudio2?apikey=${apikey}&url=https://www.youtube.com/watch?v=${result.result[0].videoId}`)
                            .then(async(result) => {
                                await itsuki.replyWithAudio({ url: result.result.link, filename: result.result.title }, { thumb: result.result.thumbnail })
                            })
                    })
                break
            case 'statusbot':
                await itsuki.reply(`----[Status Bot]----
...
`)
                break
            case 'ytsearch':
                if (args.length == 0) return await reply(`Example: ${prefix + command} Wuhan Kontol`)
                try {
                    query = args.join(" ")
                    ...
                    hasil.forEach(async(res) => {
                        ...
                        await itsuki.replyWithPhoto({ url: res.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                    })
                } catch (e) {
            ...
            case 'ytmp3':
                ...
                await itsuki.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                if (Number(result.size.split(` MB`)[0]) >= 50.00) return await reply(`Sorry the bot cannot send more than 50 MB!`)
                await itsuki.replyWithAudio({ url: result.link, filename: result.title })
                break
            case 'ytmp4':
                if (args.length == 0) return await reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
                result = await fetchJson(`https://api.lolhuman.xyz/api/ytaudio2?apikey=${apikey}&url=${ args[0]}`)
                ...
                await itsuki.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                if (Number(result.size.split(` MB`)[0]) >= 50.00) return await reply(`Sorry the bot cannot send more than 50 MB!`)
                await itsuki.replyWithVideo({ url: result.link })
                break
            case 'tiktoknowm':
                ...
                await itsuki.replyWithVideo({ url: result.result })
                break
            case 'tiktokmusic':
                if (args.length == 0) return await reply(`Example: ${prefix + command} https://vt.tiktok.com/ZSwWCk5o/`)
                await itsuki.replyWithAudio({ url: `https://api.lolhuman.xyz/api/tiktokmusic?apikey=${apikey}&url=${args[0]}` })
                break
            case 'twitterimage':
                ...
                result = await fetchJson(url)
                await itsuki.replyWithPhoto({ url: result.result.link }, { caption: result.result.title })
                break
            case 'twittervideo':
                ...
                result = await fetchJson(url)
                await itsuki.replyWithVideo({ url: result.result.link[0].url })
                break
            case 'spotify':
                ...
                await itsuki.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                await itsuki.replyWithAudio({ url: result.link }, { title: result.title, thumb: result.thumbnail })
                break
            case 'spotifysearch':
                if (args.length == 0) return await reply(`Example: ${prefix + command} Melukis Senja`)
                try {
            ...
            case 'jooxplay':
                ...
                if ((caption + result.lirik).length >= 1024) {
                    await itsuki.replyWithPhoto({ url: result.image }, { caption: caption, parse_mode: "Markdown" })
                    await itsuki.replyWithMarkdown(result.lirik)
                } else {
                    await itsuki.replyWithPhoto({ url: result.image }, { caption: caption + result.lirik, parse_mode: "Markdown" })
                }
                await itsuki.replyWithAudio({ url: result.audio[0].link, filename: result.info.song }, { thumb: result.image })
                break
            case 'zippyshare':
            ...
            case 'pinterest':
                ...
                url = url.result
                await itsuki.replyWithPhoto({ url: url })
                break
            case 'pinterestdl':
            ...
            case 'pixiv':
                if (args.length == 0) return await reply(`Example: ${prefix + command} loli kawaii`)
                query = args.join(" ")
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/pixiv?apikey=${apikey}&query=${query}` })
                break
            case 'pixivdl':
                if (args.length == 0) return await reply(`Example: ${prefix + command} 63456028`)
                pixivid = args[0]
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/pixivdl/${pixivid}?apikey=${apikey}` })
                break
            ...
            case 'reverse':
                ...
                await itsuki.reply(`Found result`, options)
                break
            ...
            case 'character':
                ...
                await itsuki.replyWithPhoto({ url: result.image.large }, { caption: text })
                break
            case 'manga':
                ...
                await itsuki.replyWithPhoto({ url: result.coverImage.large }, { caption: text })
                break
            case 'anime':
                if (args.length == 0) return await reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
                query = args.join(" ")
            ...
            case 'wait':
                if (isQuotedImage || isQuotedAnimation || isQuotedVideo || isQuotedDocument) {
                    ...
                    await itsuki.replyWithVideo({ url: result.video }, { caption: text })
                } else {
            ...
            case 'kusonime':
                ...
                if (text.length <= 1024) {
                    return await itsuki.replyWithPhoto({ url: result.thumbnail }, { caption: text })
                }
                await itsuki.replyWithPhoto({ url: result.thumbnail })
                await reply(text)
                break
            case 'kusonimesearch':
                if (args.length == 0) return await reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
            ...
            case 'lk21':
                ...
                await itsuki.replyWithPhoto({ url: result.thumbnail }, { caption: text })
                break
            case 'drakorongoing':
            ...
            case 'wattpad':
                ...
                text += `Story : \n${result.story}`
                await itsuki.replyWithPhoto({ url: result.photo }, { caption: text })
                break
            case 'wattpadsearch':
            ...
            case 'quotesdilan':
                ...
                break
            case 'quotesimage':
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/random/${command}?apikey=${apikey}` })
                break
            case 'faktaunik':
            case 'katabijak':
            case 'pantun':
            ...
            case 'hentai4everyone':
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=${apikey}` })
                break
            case 'bj':
            case 'ero':
            case 'cum':
            ...
            case 'kemonomimi':
            case 'nsfw_avatar':
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/random2/${command}?apikey=${apikey}` })
                break
            ...
            case 'thunder':
                ...
                text = args.join(" ")
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${text}` })
                break
            case 'pornhub':
            case 'glitch':
            case 'avenger':
            ...
            case 'wallgravity':
                if (args.length == 0) return await reply(`Example: ${prefix + command} LoL Human`)
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}` })
                break
            ...
            case 'carvedwood':
                if (args.length == 0) return await reply(`Example: ${prefix + command} LoL Human`)
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args.join(" ")}` })
                break
            case 'owner':
                reply('@IAmAdyy nih owner gua')
                break
            ...
            case 'pubg':
                if (args.length == 0) return await reply(`Example: ${prefix + command} LoL Human`)
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}` })
                break
            ...
            case 'lolbanner':
            case 'avatardota':
                if (args.length == 0) return await reply(`Example: ${prefix + command} Itsuki`)
                await itsuki.replyWithPhoto({ url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args.join(" ")}` })
                break
            case 'test':
                test = await bot.telegram.getChatMembersCount(itsuki.message.chat.id)
                console.log(test)
                break
            default:
...
bot.telegram.getMe().then((getme) => {
    ...
    console.log(chalk.greenBright(' 「 TELEGRAM BOT 」'))
